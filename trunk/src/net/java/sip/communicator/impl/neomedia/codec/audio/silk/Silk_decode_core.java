 ...
 * 
 * Class "Silk_decode_core" is mainly based on 
 *../SILK_SDK_SRC_FLP_v1.0.6/src/SKP_Silk_decode_core.c
 */
...
//	    short   *A_Q12, *B_Q14, *pxq, A_Q12_tmp[ MAX_LPC_ORDER ];
	    short[] A_Q12;
	    short[] B_Q14;
	    int     B_Q14_offset;
...
//	    int   Gain_Q16, *pred_lag_ptr, *pexc_Q10, *pres_Q10, LTP_pred_Q14, LPC_pred_Q10;
	    int   Gain_Q16;
	    int[] pred_lag_ptr;
...
//	    	rand_sedd = SKP_RAND( rand_seed );
	        rand_seed = Silk_SigProc_FIX.SKP_RAND( rand_seed );
	        /* dither = rand_seed < 0 ? 0xFFFFFFFF : 0; */
//djinn //     dither = SKP_RSHIFT( rand_seed, 31 );
	        dither = rand_seed>>31;
	    
//	        psDec.exc_Q10[ i ] = SKP_LSHIFT( ( int )q[ i ], 10 ) + offset_Q10;
...
//djinn ???	    pxq      = &psDec.outBuf[ psDec.frame_length ];
	    pxq      = psDec.outBuf;
...
//djinn ??	        SKP_memcpy( A_Q12_tmp, A_Q12, psDec.LPC_order * sizeof( short ) ); 
	        System.arraycopy(A_Q12, 0, A_Q12_tmp, 0, psDec.LPC_order);
	        
//djinn ??	        B_Q14         = &psDecCtrl.LTPCoef_Q14[ k * LTP_ORDER ];
...
//djinn ???	           SKP_memset( B_Q14, 0, LTP_ORDER * sizeof( short ) );
	        	Arrays.fill(B_Q14, B_Q14_offset, B_Q14_offset+Silk_define.LTP_ORDER, (short)0);
	        	
//	        	B_Q14[ LTP_ORDER/2 ] = ( short )1 << 12; /* 0.25 */
...
//	                SKP_memset( FiltState, 0, psDec.LPC_order * sizeof(int) ); /* Not really nessacary, but Valgrind will complain otherwise */
...
//	                inv_gain_Q32 = SKP_LSHIFT( inv_gain_Q16, 16 );
	                ...
	                if( k == 0 ) {
	                    /* Do LTP downscaling */
//	                    inv_gain_Q32 = SKP_LSHIFT( Silk_macros.SKP_SMULWB( inv_gain_Q32, psDecCtrl.LTP_scale_Q14 ), 2 );
	        ...
	        if( sigtype == Silk_define.SIG_TYPE_VOICED ) {
	            /* Setup pointer */
//djinn ??	            pred_lag_ptr = &psDec.sLTP_Q16[ psDec.sLTP_buf_idx - lag + LTP_ORDER / 2 ];
...
//	                pres_Q10[ i ] = SKP_ADD32( pexc_Q10[ i ], Silk_SigProc_FIX.SKP_RSHIFT_ROUND( LTP_pred_Q14, 4 ) );
...
//	            SKP_memcpy( pres_Q10, pexc_Q10, psDec.subfr_length * sizeof( int ) );
...
//	        pxq      += psDec.subfr_length;
	    ...
	    /* Copy to output */
//	    SKP_memcpy( xq, &psDec.outBuf[ psDec.frame_length ], psDec.frame_length * sizeof( short ) );
