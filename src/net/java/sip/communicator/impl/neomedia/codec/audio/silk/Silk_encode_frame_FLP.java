...
//	    SKP_memcpy( pIn_HP, pIn, psEnc->sCmn.frame_length * sizeof( SKP_int16 ) );
	    for(int i_djinn=0; i_djinn<psEnc.sCmn.frame_length; i_djinn++)
	    	pIn_HP[i_djinn] = pIn[pIn_offset+i_djinn];
	    }
//	#endif
...
//	    SKP_memcpy( pIn_HP_LP, pIn_HP, psEnc->sCmn.frame_length * sizeof( SKP_int16 ) );
	    for(int i_djinn=0; i_djinn<psEnc.sCmn.frame_length; i_djinn++)
	    	pIn_HP_LP[i_djinn] = pIn_HP[i_djinn];
	    }
//	#endif
	    ...
	    /************************/
	    Silk_noise_shape_analysis_FLP.SKP_Silk_noise_shape_analysis_FLP( psEnc, sEncCtrl, res_pitch_frame, x_frame );
	    ...
	    /****************************************/
	    nBytesLBRR = Silk_define.MAX_ARITHM_BYTES;
...
//	        ( psEnc->sCmn.frame_length + psEnc->sCmn.la_shape ) * sizeof( SKP_float ) );
	    for(int i_djinn=0; i_djinn< psEnc.sCmn.frame_length + psEnc.sCmn.la_shape; i_djinn++)
	    	psEnc.x_buf[psEnc.x_buf_offset+i_djinn] = psEnc.x_buf[psEnc.x_buf_offset+psEnc.sCmn.frame_length+i_djinn];
	    ...
	    if( psEnc.sCmn.nFramesInPayloadBuf * Silk_define.FRAME_LENGTH_MS >= psEnc.sCmn.PacketSize_ms ) 
	    {
	        ...
	        if( pnBytesOut[0] >= nBytes ) 
	        {
	            ...
	            if( frame_terminator > Silk_define.SKP_SILK_MORE_FRAMES && 
	                    pnBytesOut[0] >= nBytes + psEnc.sCmn.LBRR_buffer[ LBRR_idx ].nBytes ) 
	            {
	...
	)
	{
	    ...
	    int     k, /*TempGainsIndices[ NB_SUBFR ],*/ frame_terminator;
	    int[] TempGainsIndices = new int[ Silk_define.NB_SUBFR ];
